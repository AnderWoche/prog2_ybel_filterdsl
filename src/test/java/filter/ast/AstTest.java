...
public class AstTest {
  ...
  @Test
  void notNegation() {
    var expected = new Expr.Not(new Expr.Comparison("artist", CompOp.EQ, new Value.Str("Beatles")));
    ...
    var expected =
        new Expr.Or(
            ...
            new Expr.And(
                ...
                new Expr.Not(new Expr.Comparison("artist", CompOp.EQ, new Value.Str("Beatles")))));
    assertAst(
        expected, "genre in (\"rock\", \"jazz\") or year <= 1990 and not artist == \"Beatles\"");
  }
  ...
  void bothBuildersAgreeOnComplexQuery() {
    ...
    assertEquals(AstPrinter.toString(visitor(query)), AstPrinter.toString(pattern(query)));
  }
}
